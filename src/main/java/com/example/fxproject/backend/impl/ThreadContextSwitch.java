...
public class ThreadContextSwitch implements Test {
    public double runCpp(){
        Path memoryAllocationExePath = getResourcePath("/com/example/fxproject/testUtils/threadContextSwitch.cpp");
    ...
    public double runPython(){
        Path memoryAllocationExePath = getResourcePath("/com/example/fxproject/testUtils/python/threadContextSwitch.py");
        if ( memoryAllocationExePath == null){
            System.out.println("Null path.");
            return -1;
        }
    ...
    public double[] runJava(int givenIterations){
        return runJava(JavaBenchmark.class.getSimpleName() + ".testContextSwitch", givenIterations);
    }
